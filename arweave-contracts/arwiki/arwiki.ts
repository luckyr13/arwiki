...
interface ArWikiPage {
  ...
  content: string;
  sponsor: string;
  updates: { at: number, tx: string, value: number, approvedBy: string }[],
  upvotes: number;
  category: string;
  downvotes: number;
  pageRewardAt: number;
...
interface ArWikiState {
  name: string;
  pages: Record<string, Record<string, ArWikiPage>>;
  roles: Record<string, string>;
  vault: Record<string, { balance: number, end: number, start: number, lang?: string, slug?: string, action?: string }[]>;
  stakes: Record<string, Record<string, Record<string, number>>>;
  ticker: string;
  balances: Record<string, number>;
  settings: Array<[string, number|string]>;
  languages: Record<string, { code: string, active: boolean, iso_name: string, native_name: string, writing_system: string }>;
}
...
class ArWiki {
  ...
  public handle(state: ArWikiState, action: any) {
    const settings = new Map<string, number|string>(state.settings);
    ...
    if (option === "transfer") {
      ...
      return this.transfer(caller, qty, target, state);
    }
    if (input.function === "balance") {
      const target = input.target || caller;
      return this.balance(target, state);
    }
    if (input.function === "unlockedBalance") {
      const target = input.target || caller;
      return this.unlockedBalance(target, state);
    }
    if (input.function === "lock") {
      const qty = input.qty;
      const lockLength = input.lockLength;
      return this.lock(qty, lockLength, caller, state);
    }
    if (input.function === "increaseVault") {
      const lockLength = input.lockLength;
      const id = input.id;
      return this.increaseVault(lockLength, settings, caller, id, state);
    }
    if (input.function === "unlock") {
      return this.unlock(caller, state);
    }
  ...
  public calculate_total_supply(vault, balances, stakes)
  {
  ...
  public get_vaultBalance(vault, caller, end) {
    let vaultBalance = 0;
  ...
  public transfer(
    ...
    state: ArWikiState)
  {
    const balances: Record<string, number> = state.balances;
  ...
  public balance(target: string, state: ArWikiState)
  {
  ...
  public unlockedBalance(target: string, state: ArWikiState)
  {
    const balances = state.balances;
    if (typeof target !== "string") {
      throw new ContractError("Must specificy target to get balance for.");
  ...
  public lock(qty: number, lockLength: number, caller: string, state: ArWikiState)
  {
    const balances = state.balances;
    const vault = state.vault;
    if (!Number.isInteger(qty) || qty <= 0) {
  ...
  public increaseVault(
    lockLength: number,
    settings: Map<string, number|string>,
    caller: string,
    id: string,
    state: ArWikiState)
  {
  ...
  public unlock(caller: string, state: ArWikiState) {
    const vault = state.vault;
    const balances = state.balances;
...
export function handle(state, action) {
  const arwiki = new ArWiki();
  return arwiki.handle(state, action);
}
